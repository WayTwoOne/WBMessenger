...
struct CountryCodeView: View {
    ...
    var filterCountry: [String:String] {
        guard !search.isEmpty else { return countryDictionary }
        return countryDictionary.filter { $0.key.localizedCaseInsensitiveContains(search)}
    }
    ...
    var body: some View {
        GeometryReader { geo in
                List(self.filterCountry.sorted(by: <), id: \.key) { key , value in
                    HStack {
...
struct CountryCodeView_Previews: PreviewProvider {
    static var previews: some View {
        CountryCodeView(countryCode: .constant("+7"), countryFlag: .constant("🇷🇺"), y: .constant(150), search: .constant(""))
    }
}
